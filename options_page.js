...
var fillOptions = function() {
  ...
  $("#greengarden_host_port_input").val(options.greengarden_host_port);
  ...
  Greengarden.ServerModel.workspaces(function(workspaces) {
    ...
    $("#domains_group").html(
        '<div>Error loading workspaces. Verify the following:<ul>' +
            '<li>Greengarden Host is configured correctly.</li>' +
            '<li>You are <a target="_blank" href="' +
            Greengarden.Options.loginUrl() +
            '">logged in</a>.</li>' +
            '<li>You have access to the Greengarden API.</li></ul>');
  });
};
...
var resetOptions = function() {
  Greengarden.Options.resetOptions();
  fillOptions();
  setSaveEnabled(false);
};

var saveOptions = function() {
  var greengarden_host_port = $("#greengarden_host_port_input").val();
  var default_domain_input = $("input[@name='default_domain_id']:checked");
  Greengarden.Options.saveOptions({
    greengarden_host_port: greengarden_host_port,
