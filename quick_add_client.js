 ...
 * does the job.
 */
Greengarden.QuickAddClient = {

  _tab_down_time: 0,

  keyDown: function(e) {
    var self = Greengarden.QuickAddClient;
    if (e.keyCode === 9) {
  ...
  listen: function() {
    // Don't run against Greengarden, which already has a QuickAdd feature. ;)
    if (!/^https?:\/\/localhost/.test(window.location.href)) {
      window.addEventListener("keydown", Greengarden.QuickAddClient.keyDown, true);
      window.addEventListener("keyup", Greengarden.QuickAddClient.keyUp, true);
    }
  }
};

Greengarden.QuickAddClient.listen();
